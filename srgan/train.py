...
def evaluate():
    ...
    for file in os.listdir(config.VALID.hr_img_path):
        print(file)

        valid_lr_img = tlx.vision.load_image(path=os.path.join(config.VALID.hr_img_path,file))
        valid_lr_img = np.asarray(valid_lr_img)
	...
	#print('lr_'+file+'_SRGAN.png')
        #tlx.vision.save_image(out[0], file_name='lr_'+file+'_SRGAN.png', path=save_dir)
        ...
        #img = cv2.imread(os.path.join(save_dir, file+'_SRGAN.png'), 0)
        img = cv2.blur(out[0], (5,5))
        cv2.imwrite(os.path.join(save_dir, 'lr_'+file.replace(".png","")+'_SRGAN.png'), img)
